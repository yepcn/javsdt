...
def find_jav_html_on_db(jav, url_db, proxy_db):
    ...
    if '仓库' in jav.name:
        ...
        if url_appointg:
            ...
            if not re.search(r'頁面未找到', html_jav_db):
                return StatusScrape.success, jdvdb
            else:
                return StatusScrape.db_specified_url_wrong, ''
        else:
            # 指定的javlibrary网址有错误
            return StatusScrape.db_specified_url_wrong, ''
...
def re_db(url_db, javdb, proxy_db):
