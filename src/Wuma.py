...
from Picture import crop_poster_baidu, crop_poster_default
# ################################################## 独特 ##########################################################
from Car import find_car_wuma
from Functions.Web.Javbus import get_bus_html
        ...
        for jav in list_jav_struct:
            ...
            try:
                ...
                if '公交车' in jav.name:
                    ...
                    if url_appointg:
                        url_jav = f'{url_bus}{url_appointg.group(1)}'
                    else:
                        num_fail += 1
                        record_fail(f'    >第{num_fail}个失败！你指定的javbus网址有错误: {path_relative}\n')
                        continue  # 【退出对该jav的整理】
                # 用户没有指定网址，则去搜索
                else:
                    # javbus的搜索url中以%20代替-_ 等字符
                    car_search = jav.car.replace("-", "%20").replace("_", "%20").replace(" ", "%20")
                    url_search = f'{url_bus}/uncensored/search/{car_search}&type=&parent=uc'
                    print('    >搜索车牌: ', url_search)
                    # 得到javbus搜索网页html
                    html_web = get_bus_html(url_search, proxy_bus)
                    # 尝试找movie-box
                    ...
                    if list_search_results:  # 搜索结果页面只有一个box
                        ...
                        jav_pref = jav_prefg.group(1) if jav_prefg else ''
                        jav_sufg = re.search(r'[^\d](\d\d+)', jav.car)
                        jav_suf = jav_sufg.group(1).lstrip('0') if jav_sufg else ''
                        ...
                        for i in list_search_results:
                            ...
                            url_suf = url_sufg.group(1).lstrip('0') if url_sufg else ''  # 匹配box上影片url，车牌的后缀数字，去除多余的0
                            # print(url_end, url_suf)
                            if jav_suf == url_suf:  # 数字相同
                                url_prefg = re.search(r'([A-Z0-9]+)[-_]?', url_end)
                                url_pref = url_prefg.group(1).upper() if url_prefg else ''
                                if jav_pref == url_pref:  # 数字相同的基础下，字母也相同，即可能车牌相同
                                    list_fit_results.append(i)
                        # 无码搜索的结果一个都匹配不上
                        if not list_fit_results:
                            num_fail += 1
                            record_fail(f'    >第{num_fail}个失败！javbus无码找不到该车牌的信息: {jav.car}，{path_relative}\n')
                            continue  # 【退出对该jav的整理】
                        # 默认用第一个搜索结果
                        url_jav = list_fit_results[0]
                        # print('最终链接: ', url_jav)
                        # print('最终list: ', list_fit_results)
                        if len(list_fit_results) > 1:
                            num_fail += 1
                            record_fail(f'    >第{num_fail}个警告！javbus搜索到同车牌的不同视频: {jav.car}，{path_relative}\n')
                    # 找不到box
                    else:
                        num_fail += 1
                        record_fail(f'    >第{num_fail}个失败！javbus无码找不到该车牌的信息: {jav.car}，{path_relative}\n')
                        continue  # 【跳出对该jav的整理】
                # 经过上面的三种情况，可能找到了jav在bus上的网页链接url_jav
                print(f'    >获取信息: {url_jav}')
                # 得到最终的jav所在网页
                html_web = get_bus_html(url_jav, proxy_bus)
                ...
                try:
                    ...
                    num_fail += 1
                    record_fail(f'    >第{num_fail}个失败！发现新的特征需要添加至【特征对照表】: {error}\n')
                    continue
                # print(genres)
                #######################################################################
                ...
                if settings.bool_nfo:
                    ...
                    f.write(f'<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?>\n'
                            ...
                            f'  <director>{dict_data["导演"]}</director>\n'
                    ...
                    if settings.bool_genre:
                        ...
                        if settings.bool_write_series and series:
                            f.write(f'  <genre>系列:{series}</genre>\n')
                ...
                if settings.bool_sculpture and jav.episode == 1:
                    if actors[0] == '有码演员':
                        ...
                        collect_sculpture(actors, jav.dir_current)
