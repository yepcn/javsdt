...
from Functions.Requests.JavdbReq import get_db_search_json, get_db_search_html
from Functions.Requests.ArzonReq import steal_arzon_cookies, find_plot_arzon


#  main开始
print('1、避开21:00-1:00，访问javdb和arzon很慢。\n'
      '2、若一直打不开javdb，请在ini中更新防屏蔽网址\n')
...
# 优化特征的字典
dict_genre = better_dict_genre('Javdb有码', to_language)
        ...
        for jav in list_jav_struct:
            ...
            try:
                ...
                # 用户没有指定网址，则去搜索
                    ...
                    url_search_web = url_db + "videos/search_autocomplete.json?q=" + jav.car
                ...
                # 得到最终的jav所在网页
                html_web = get_db_search_html(url_on_web, proxy_db)
                # print(html_web)
                ...
                if str(coverg) != 'None':
                    ...
                    url_cover = ''
                # 发行日期
                premieredg = re.search(r'<span class="value">(\d\d\d\d-\d\d-\d\d)', html_web)
                ...
                seriesg = re.search(r'series/.+?">(.+?)<', html_web)
                if str(seriesg) != 'None':
                ...
                # 1重命名视频【相同】
                try:
                    dict_data, jav, num_temp = rename_mp4(jav, num_fail, settings, dict_data, list_name_video, path_relative, str_cd)
                    num_fail = num_temp
                ...
                try:
                    jav, num_temp = classify_files(jav, num_fail, settings, dict_data, list_classify_basis, root_classify)
                    num_fail = num_temp
                except FileExistsError:
                    num_fail += 1
                    continue
                ...
                try:
                    jav, num_temp = rename_folder(jav, num_fail, settings, dict_data, list_name_folder, bool_separate_folder, num_all_episodes)
                    num_fail = num_temp
                except FileExistsError:
                    num_fail += 1
                    continue
